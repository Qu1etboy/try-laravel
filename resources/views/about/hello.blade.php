...
<body>
  ...
  <h1 class="text-4xl text-pink-500 font-bold">ID: {{ $id }}</h1>
  <h2 class="text-2xl font-semibold">Name: {{ $info['name'] }}</h2>
  <div>Address: {{ $info['address']}}</div>

  <div class="text-xs">{{ now()->diffForHumans() }}</div>

  <a href="{{ url('/about/history') }}" class="underline hover:text-pink-500">History</a>
  <a href="{{ route('about') }}" class="underline hover:text-pink-500">About</a>
</body>
